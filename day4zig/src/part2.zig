...
pub fn main() !void {
    ...
    draw_map(rows);
    while (true) {
        const papers = count_paper(allocator, rows) catch return aoc_utils.AocError.OutOfMemory;
        if (papers.len == 0) {
            break;
        }
        removed += papers.len;
...
fn count_paper(allocator: std.mem.Allocator, map: Map) AllErrors![]Paper {
    ...
    for (map.items, 0..) |row_chars, r_idx| {
        for (row_chars, 0..) |char_at_col, c_idx| {
            if (char_at_col == '@') {
                ...
                if (count < 4) {
                    _ = list.append(allocator, Paper{ .row = r_idx, .col = c_idx }) catch return aoc_utils.AocError.OutOfMemory;
                }
            }
        }
    }
